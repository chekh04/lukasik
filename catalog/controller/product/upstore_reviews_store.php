<?php
class ControllerProductUpstoreReviewsStore extends Controller {
	public function index() {
		...
		$this->upstore_minifier->addScript('catalog/view/theme/upstore/js/showmore.js','footer');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('product/upstore_reviews_store', $url)
		);
		...
		$reviews_total = $this->model_catalog_upstore_reviews_store->getTotalReviewsStore();

		$data['totals_reviews'] = $reviews_total;
		...
		$data['sorts'][] = array(
			...
			'href'  => $this->url->link('product/upstore_reviews_store', 'sort=date_added_desc' . $url)
		);
		$data['sorts'][] = array(
			'text'  => $this->language->get('text_date_added_asc'),
			'value' => 'date_added_asc',
			'href'  => $this->url->link('product/upstore_reviews_store', 'sort=date_added_asc' . $url)
		);
		...
		foreach($limits as $value) {
			$data['limits'][] = array(
				'text'  => $value,
				'value' => $value,
				'href'  => $this->url->link('product/upstore_reviews_store', $url . '&limit=' . $value)
			);
		}
		...
		$pagination->url = $this->url->link('product/upstore_reviews_store', $url . '&page={page}');
		...
		if ($page == 1) {
		    $this->document->addLink($this->url->link('product/upstore_reviews_store', '', 'SSL'), 'canonical');
		} elseif ($page == 2) {
		    $this->document->addLink($this->url->link('product/upstore_reviews_store', '', 'SSL'), 'prev');
		} else {
		    $this->document->addLink($this->url->link('product/upstore_reviews_store', 'page='. ($page - 1), 'SSL'), 'prev');
		}

		if ($limit && ceil($reviews_total / $limit) > $page) {
		    $this->document->addLink($this->url->link('product/upstore_reviews_store', 'page='. ($page + 1), 'SSL'), 'next');
		}
		...
		$this->response->setOutput($this->load->view('product/upstore_reviews_store', $data));
		} else {
			$url = '';
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$all_review_theme = $this->model_catalog_upstore_reviews_store->getAllReviewTheme();

			foreach ($all_review_theme as $result) {
				if(!isset($rating[$result['reviews_store_theme_id']])) {
					$json['error']['themes'][] = [
						'id' => $result['reviews_store_theme_id'],
					];
				}
			}

			if (!empty($json['error']['themes'])) {
				$json['error']['review_store_rating'] = $this->language->get('error_rating');
			}

			$reviews_store_setting = $this->config->get('reviews_store_setting');

			if(isset($reviews_store_setting['captcha']) && ($reviews_store_setting['captcha'] == 1) && $this->config->get('captcha_' . $this->config->get('config_captcha') . '_status')){
			...
			if (!isset($json['error'])) {
				$this->model_catalog_upstore_reviews_store->addReviewStore($this->request->post);
	...
	public function popupFormReviewStore() {
		...
		$data['reviews_theme'] = $this->model_catalog_upstore_reviews_store->getReviewsTheme();
		$data['reviews_store_setting'] = $this->config->get('reviews_store_setting');

		if(isset($data['reviews_store_setting']['captcha']) && ($data['reviews_store_setting']['captcha'] == 1) && $this->config->get('captcha_' . $this->config->get('config_captcha') . '_status')){
		...
		$this->response->setOutput($this->load->view('product/upstore_reviews_store_form', $data));
	}
	...
	public function likeDislike() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$this->load->model('catalog/upstore_reviews_store');
			$reviews_store_id = $this->model_catalog_upstore_reviews_store->addLikeDislike($this->request->post);
			$json = $this->model_catalog_upstore_reviews_store->getLikeDislike($reviews_store_id);
		}
