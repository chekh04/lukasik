<?php
class ControllerExtensionModuleUpstoreQuestionAnswer extends Controller {
	public function getList() {
		$this->load->language('extension/module/upstore_question_answer');
		...
		if (isset($data['qadata']['captcha_field']) && ($data['qadata']['captcha_field'] == 1) && $this->config->get('captcha_' . $this->config->get('config_captcha') . '_status')) {
			$data['captcha_qa'] = $this->load->controller('extension/captcha/' . $this->config->get('config_captcha'));
		} else {
			$data['captcha_qa'] = '';
		}
		...
		$this->load->model('extension/module/upstore_question_answer');
		...
		$question_answer_total = $this->model_extension_module_upstore_question_answer->getTotalQuestionAnswer($this->request->get['product_id']);

		$results = $this->model_extension_module_upstore_question_answer->getQuestionAnswer($this->request->get['product_id'], ($page - 1) * 5, 5);
		...
		$pagination->limit = 5;
		$pagination->url = $this->url->link('extension/module/upstore_question_answer/getList', 'product_id=' . $data['product_id'] . '&page={page}');
		...
		if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			$this->response->setOutput($this->load->view('extension/module/upstore_question_answer', $data));
		} else {
			return $this->load->view('extension/module/upstore_question_answer', $data);
		}
	}
	...
	public function validateForm() {
		if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				...
				if (isset($qadata['name_field']) && isset($qadata['name_field_required']) && $qadata['name_field'] == 1 && $qadata['name_field_required'] == 1) {
				...
				if (isset($qadata['telephone_field']) && isset($qadata['telephone_field_required']) && $qadata['telephone_field'] == 1 && $qadata['telephone_field_required'] == 1) {
				...
				if (isset($qadata['email_field']) && isset($qadata['email_field_required']) && $qadata['email_field'] == 1 && $qadata['email_field_required'] == 1) {
				...
				if (isset($qadata['captcha_field']) && ($qadata['captcha_field'] == 1) && $this->config->get('captcha_' . $this->config->get('config_captcha') . '_status')) {
				...
				if (!isset($json['error'])) {
					...
					$this->model_extension_module_upstore_question_answer->SaveQuestionAnswer($data_save_post);
