<?php
class ControllerExtensionModuleUpstoreFoundCheaperProduct extends Controller {
	public function index() {
		if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			...
			$this->load->language('extension/module/upstore_found_cheaper_product');
			...
			$this->response->setOutput($this->load->view('extension/module/upstore_found_cheaper_product', $data));
		} else {
	...
	public function found_cheaper_product_confirm() {
		if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			$json = array();
			$this->load->language('extension/module/upstore_found_cheaper_product');
			$this->load->model('extension/module/upstore_found_cheaper_product');
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['action'])) {
				...
				$fcpdata = (array)$this->config->get('fcpdata');
				if (isset($fcpdata['name_field']) && isset($fcpdata['name_field_required']) && $fcpdata['name_field'] == 1 && $fcpdata['name_field_required'] == 1) {
				...
				if (isset($fcpdata['telephone_field']) && isset($fcpdata['telephone_field_required']) && $fcpdata['telephone_field'] == 1 && $fcpdata['telephone_field_required'] == 1) {
				...
				if (isset($fcpdata['link_field']) && isset($fcpdata['link_field_required']) && $fcpdata['link_field'] == 1 && $fcpdata['link_field_required'] == 1) {
					if (empty($this->request->post['link_field'])) {
						$json['error']['link_field'] = $this->language->get('error_link_field');
					}
				}
				if (isset($fcpdata['email_field']) && isset($fcpdata['email_field_required']) && $fcpdata['email_field'] == 1 && $fcpdata['email_field_required'] == 1) {
				...
				if (isset($fcpdata['comment_field']) && isset($fcpdata['comment_field_required']) && $fcpdata['comment_field'] == 1 && $fcpdata['comment_field_required'] == 1) {
			...
			// Agree to terms
				...
				if (!isset($json['error'])) {
					...
					$this->model_extension_module_upstore_found_cheaper_product->SaveFoundCheaper($data_save_post);
