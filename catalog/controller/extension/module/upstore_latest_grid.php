<?php
class ControllerExtensionModuleUpstoreLatestGrid extends Controller {
	...
	public function index($setting) {
		...
		if(deviceType == 'tablet') {
			$setting['limit'] = (int)$setting['limit_tablet'];
		}
		if(deviceType == 'computer') {
			$setting['limit'] = (int)$setting['limit'];
		}
		...
		$setting['category_id'] = isset($setting['category_id']) ? (int)$setting['category_id'] : 0;
		...
		$this->load->language('extension/module/upstore_latest_grid');
		...
		if ($setting['category_id']) {
			$filter_category_id = (int)$setting['category_id'];
		} else {
		...
		$product_total = $this->model_upstore_theme->getTotalProducts($filter_data);

		if ($product_total > $limit_max) {
			$product_total = $limit_max;
		}

		$results = $this->model_upstore_theme->getLatest($filter_data);
		...
		if ($results) {
			...
			return $this->load->view('extension/module/upstore_latest_grid', $data);
		}
	}
}
